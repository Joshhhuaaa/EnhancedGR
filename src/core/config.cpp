...
void Config::Read()
{
    ...
    for (auto* setting : List())
    {
        ...
        // A value the reader cannot make sense of falls back to the default, which is otherwise
        // indistinguishable in the log from the key not being there at all.
        if (it != section.end() && !Parse(it->second, setting->kind, setting->value))
            spdlog::warn("[{}] {} not understood, keeping the default", setting->section, setting->key);
