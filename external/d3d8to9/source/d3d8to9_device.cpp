...
#include "fps_limit.hpp" // FPSLIMIT
#include "msaa.hpp" // MSAA
#include "letterbox.hpp" // LETTERBOX
#include <regex>
#include <assert.h>
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::Reset(D3DPRESENT_PARAMETERS8 *pPresentationParameters)
{
	...
	// The SMAA render targets and the MSAA depth substitute are DEFAULT-pool and make Reset fail
	// while they are alive, and both are rebuilt lazily, so they are released before every Reset.
	// Not gated on the cooperative level: the game resets from WM_ACTIVATEAPP, which d3d9's own
	// subclass delivers before it has updated that state, so a Reset that succeeds can follow a
	// TestCooperativeLevel that still says lost
	...
	// A redundant Reset can wedge a multisampled DISCARD swap chain on Windows 11: Reset and
	// Present succeed, but the screen stays black until an alt-tab cycle. The game rebuilds the
	// same mode on every menu transition, and in borderless the mode never changes at all, so
	// skip those Resets without touching the device. A lost device never takes this path
...
#ifndef D3D8TO9NOLOG
	LOG << "> Reset: cooperative level " << std::hex << deviceState << ", result " << hr << std::dec << std::endl;
#endif
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::Present(const RECT *pSourceRect, const RECT *pDestRect, HWND hDestWindowOverride, const RGNDATA *pDirtyRegion)
{
	...
	FpsLimit::OnPresent(); // FPSLIMIT

	// The game presents with a source and a dest rect on its windowed path, which is illegal
	// on the DISCARD chain MSAA forces
	if (Msaa::Active()) // MSAA
	...
	Letterbox::OnPresent(ProxyInterface); // LETTERBOX
